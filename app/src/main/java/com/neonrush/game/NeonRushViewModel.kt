...
// NOTE: LeaderboardPilot and SocialComment are defined in LeaderboardModels.kt.
// Do NOT redeclare them here — this package already has one definition of each,
// and a second one in this file causes a "conflicting declarations" build error.
...
fun freezeStreak() {
    ...
    private fun loadSocialComments() {
        ...
        _socialComments.value = listOf(
            SocialComment(id = "seed_1", author = "CyberRunner", text = "Just secured a 850 run using the Matrix Grid fighter! The drafting is key in Warp Voids.", timestamp = now - 10 * 60_000L, zone = "Singularity Terminus"),
            SocialComment(id = "seed_2", author = "ZeroGlitch", text = "That freeze hazard in Zone 3 completely ruined my sliding angles. Watch your thrusters!", timestamp = now - 60 * 60_000L, zone = "Methane Basin"),
            SocialComment(id = "seed_3", author = "RetroWave", text = "Anyone else thinks the Cyan Diamond skin handles smoother on Chromium light rings?", timestamp = now - 3 * 60 * 60_000L, zone = "Chromium Grid"),
            SocialComment(id = "seed_4", author = "DriftKing_X", text = "Is the Daily Rush active? Heard it gives 50 free gems today.", timestamp = now - 4 * 60 * 60_000L, zone = "Cyber Alley"),
            SocialComment(id = "seed_5", author = "NeonVolt", text = "Unbelievable speed. I made the top 10 today but dropped immediately.", timestamp = now - 6 * 60 * 60_000L, zone = "Chromium Grid")
        )
    }
